...
def extract_contact(cv_text):
    info = {}
    for key, pattern in [("email", _EMAIL_RE), ("phone", _PHONE_RE)]:
...
def _rule_based_suggestions(cv_text, sections, contact):
    lang     = _lang(cv_text)
    ...
    if "linkedin" not in contact:
        tips.append(
            "إضافة رابط لينكد إن يزيد من فرصك." if lang == "ar"
            else "Adding a LinkedIn profile URL improves recruiter visibility."
        )

    if len(cv_text.split()) < 100:
        tips.append(
            "السيرة الذاتية قصيرة جداً، أضف المزيد من التفاصيل." if lang == "ar"
...
def analyze_cv(cv_text, use_ai=False):
